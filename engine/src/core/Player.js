...
export class Player {
  constructor(index, character) {
    ...
    this.judgeCards = [];  // 判定区的牌（乐不思蜀、盖电等）
    this.isAlive = true;
    this.hasUsedSha = false;
    this.hasUsedJiu = false;
    this.drunken = false;  // 是否喝封闭针了
  }

  // 获取攻击范围
  getAttackRange() {
    const base = this.equipment.weapon?.range || 1;
    // Stephen Curry · 射程：使用投的距离 +1
    const curryBonus = this.character?.key === 'stephen_curry' ? 1 : 0;
    return base + curryBonus;
  }
